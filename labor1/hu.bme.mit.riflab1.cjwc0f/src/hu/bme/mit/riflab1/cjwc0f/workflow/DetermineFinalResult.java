...
public class DetermineFinalResult {
	...
	public static ApplicationData decide(ApplicationData applicantData,
		...
		if (applicantData.getResult() != null) {
			...
			applicantData.setRoomNumber(applicantData.getResult()
					.getRoomNumber());
		} else {
			if (applicantData.getSocialResult() != null
					&& applicantData.getSocialResult().isAdmitted()) {
				// If admitted based only on the social results
				applicantData.setAdmitted(true);
				applicantData.setRoomNumber(applicantData.getSocialResult()
						.getRoomNumber());
			} else {
				// Not admitted
				applicantData.setAdmitted(false);
				applicantData.setRoomNumber(0);
			}
		}
